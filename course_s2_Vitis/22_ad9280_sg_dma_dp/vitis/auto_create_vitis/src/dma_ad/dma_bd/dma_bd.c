...
int CreateBdChain(u32 *BdDesptr, u16 BdCount, u32 TotalByteLen, u8 *DmaBufferPtr, u32 Direction)
{
	...
	for(i = 0 ; i < BdCount ; i++)
	{
		...
		/* Write to Next descriptor point Higher 32bit */
		XAxiDma_BdWrite(BdPtrCurr, XAXIDMA_BD_NDESC_MSB_OFFSET, 0x0) ;
		/* Write to Buffer Address Lower 32bit */
		XAxiDma_BdWrite(BdPtrCurr, XAXIDMA_BD_BUFA_OFFSET, (u32)RxBufCurr) ;
		/* Write to Buffer Address Higher 32bit */
		XAxiDma_BdWrite(BdPtrCurr, XAXIDMA_BD_BUFA_MSB_OFFSET, 0x0) ;
...
int Bd_Start(u32 *BdDesptr, u16 BdCount, XAxiDma *XAxiDmaPtr, u32 Direction)
{
	...
	XAxiDma_BdWrite(RingPtr->ChanBase, XAXIDMA_CDESC_MSB_OFFSET, 0x0) ;
	/* Start DMA */
	...
	if (XAxiDma_BdRingHwIsStarted(RingPtr))
		...
		XAxiDma_BdWrite(RingPtr->ChanBase, XAXIDMA_TDESC_MSB_OFFSET, 0x0) ;
